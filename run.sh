...
if [ "$1" == "--install" ]; then
    ...
    echo -e "\033[38;5;46m[*] Installation des paquets système essentiels (Tshark, SSDEEP)...\033[0m"
    sudo apt-get install -y python3-pip python3-dev python3-venv build-essential libffi-dev libssl-dev ssdeep libfuzzy-dev tshark
...
if [ "$1" == "--install" ]; then
    ...
    python3 -m pip install --disable-pip-version-check -r requirements.txt -q
fi
